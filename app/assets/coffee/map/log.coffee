...
$('.log-header').on('mousedown touchstart', (e)->
	...
	$(document).on('mousemove touchmove', (e)->
		if dragging
			...
			if logHeight < 0
				logHeight = 0
			$logWindow.css('height', logHeight)
...
ps2maps.logFacilityControl = (facility_id, old_faction_id, new_faction_id, timestamp) ->
	...
	verb = if old_faction_id == new_faction_id then "resecured" else "captured"

	# The facility icon
	icon  = "<div class='facility-icon'>"
	icon += "<svg viewBox='0 0 256 256' class='marker-icon " + facility.facilityType + " " + faction.slug + "'>"
	console.log facility, facility.facilityType
	console.log $svgContainer.find('#' + facility.facilityType).html();
	icon += $svgContainer.find('#' + facility.facilityType).html();
	icon += "</svg></div>"
	...
	if $('#filter-' + faction.slug).is(':checked') and $('#filter-' + verb).is(':checked')
		$(html).hide().css('opacity',0).prependTo(list).slideDown('slow').animate({opacity: 1.0})
		ps2maps.logIncrement()
	else # Add item to list, don't show it
		$(html).hide().prependTo(list)
